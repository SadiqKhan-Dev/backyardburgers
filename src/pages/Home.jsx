...
function Home() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
              ...
              return (
                <div key={index} className="text-center relative">
                  ...
                  {index < howItWorks.length - 1 && (
                    <div className="hidden md:block absolute top-16 left-1/2 w-full h-1 bg-gradient-to-r from-primary-400 to-accent-400"></div>
                  )}

                  <div className="relative z-10">
                    <div className={`bg-gradient-to-br ${step.color} w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl transform hover:scale-110 transition-transform duration-300`}>
      ...
      {/* Categories Preview */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 bg-white dark:bg-gray-900">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-4 font-display">
            Explore Our Menu
          </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[
            { name: 'Burgers', icon: Beef, color: 'from-red-500 to-red-600', count: products.filter(p => p.category === 'burgers').length },
            { name: 'Chicken', icon: Drumstick, color: 'from-yellow-500 to-yellow-600', count: products.filter(p => p.category === 'chicken').length },
            { name: 'Sides', icon: Soup, color: 'from-green-500 to-green-600', count: products.filter(p => p.category === 'sides').length },
            { name: 'Drinks', icon: CupSoda, color: 'from-blue-500 to-blue-600', count: products.filter(p => p.category === 'drinks').length },
            { name: 'Desserts', icon: CakeSlice, color: 'from-pink-500 to-pink-600', count: products.filter(p => p.category === 'desserts').length },
            { name: 'Salads', icon: Salad, color: 'from-teal-500 to-teal-600', count: products.filter(p => p.category === 'salads').length },
            { name: 'Breakfast', icon: Sunrise, color: 'from-orange-500 to-orange-600', count: products.filter(p => p.category === 'breakfast').length },
            { name: 'Combos', icon: Gift, color: 'from-purple-500 to-purple-600', count: products.filter(p => p.category === 'combos').length },
          ].map((category) => {
            ...
            return (
              <Link
                ...
                className="group"
              >
                <div className={`bg-gradient-to-br ${category.color} rounded-xl p-6 text-white text-center transform hover:scale-110 transition-all duration-300 shadow-lg hover:shadow-xl`}>
                  <Icon className="w-12 h-12 mx-auto mb-3" />
                  <span className="font-semibold text-sm block mb-1">{category.name}</span>
